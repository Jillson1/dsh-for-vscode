...
// 覆盖：有选区出两个按钮、零长度选区不出、开关关闭不出、选区属于别的文档不出。
// 这些规则决定"点一下光标会不会冒出工具条"以及"按钮会不会出现在不相干的文件上"。
...
test('有选区：在选区首行出两个并排按钮（添加到 DSH + Quick Edit）', () => {
  const specs = selectionLensSpecs(
    DOC,
    { fsPath: DOC, startLine: 4, isEmpty: false },
    true,
  );
...
test('两个按钮落在同一行（并排，而不是上下两行）', () => {
  const specs = selectionLensSpecs(DOC, { fsPath: DOC, startLine: 9, isEmpty: false }, true);
  assert.equal(new Set(specs.map((s) => s.line)).size, 1);
});

test('按钮挂在选区首行，而不是末行（多行选区）', () => {
  // startLine 是选区起始行；调用方传的就是它，这里确保不会被改成末行
  const specs = selectionLensSpecs(DOC, { fsPath: DOC, startLine: 2, isEmpty: false }, true);
  assert.equal(specs[0]?.line, 2);
});

test('零长度选区（只点了一下光标）→ 不出工具条', () => {
  assert.deepEqual(selectionLensSpecs(DOC, { fsPath: DOC, startLine: 3, isEmpty: true }, true), []);
});
...
test('开关关闭 → 不出工具条（即便有选区）', () => {
  assert.deepEqual(selectionLensSpecs(DOC, { fsPath: DOC, startLine: 1, isEmpty: false }, false), []);
});

test('选区属于别的文档 → 在该文档上不出按钮（VS Code 会逐文档询问）', () => {
  const other = 'E:\\codee\\document\\other.ts';
  assert.deepEqual(selectionLensSpecs(other, { fsPath: DOC, startLine: 1, isEmpty: false }, true), []);
  assert.equal(selectionLensSpecs(DOC, { fsPath: DOC, startLine: 1, isEmpty: false }, true).length, 2);
});

test('异常负行号 → 归一到第 0 行（不产生非法 Range）', () => {
  const specs = selectionLensSpecs(DOC, { fsPath: DOC, startLine: -5, isEmpty: false }, true);
